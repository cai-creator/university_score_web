    ...
    <a-card title="个人信息" class="profile-card">
      ...
      >
        ...
        <a-form-item label="工号" name="teacherId">
          <a-input v-model:value="form.teacherId" placeholder="请输入工号" disabled />
        </a-form-item>
        <a-form-item label="院系" name="department">
          <a-input v-model:value="form.department" placeholder="请输入院系" disabled />
        </a-form-item>
...
const fetchUserInfo = async () => {
  try {
    const data = await api.teacher.getCurrentUser()
    // 填充表单数据，兼容后端API的字段名
    form.name = data.name || ''
    form.teacherId = data.school_id || data.teacher_id || data.teacherId || ''
    form.department = data.college_name || ''
    form.phone = data.phone || data.contact || ''
...
const handleSubmit = async (values) => {
  try {
    ...
    // 转换为后端API需要的数据格式，不包含院系字段（院系不能修改）
    const requestData = {
      name: values.name,
      phone: values.phone,
      email: values.email,
    ...
    // 调用API保存用户信息
    await api.teacher.updateProfile(requestData)
